"""Tests for ultimate_asr.audio.recorder.Recorder (PyAudio is mocked)."""
...
from ultimate_asr.audio.recorder import (
...
def mock_pyaudio():
    ...
    with patch("ultimate_asr.audio.recorder.pyaudio.PyAudio", return_value=fake_pa_instance):
...
class TestRecorderSilenceMode:
    ...
    def test_stream_open_failure_resets_recording_flag(self, mock_pyaudio):
        ...
        recorder = Recorder()
        with patch("ultimate_asr.audio.recorder.ensure_vad_model"), \
             patch("ultimate_asr.audio.recorder.SileroVAD") as mock_vad_cls:
            mock_vad = MagicMock()
...
class TestRecordSilenceModeSilero:
    ...
    def mock_pyaudio_stream(self):
        ...
        with patch("ultimate_asr.audio.recorder.pyaudio.PyAudio") as pa_cls:
            pa = MagicMock()
    ...
    def test_surfaces_download_failure(self, mock_pyaudio_stream):
        from ultimate_asr.audio.recorder import Recorder
    ...
    def test_resets_vad_before_use(self, mock_pyaudio_stream):
        ...
        with patch("ultimate_asr.audio.recorder.ensure_vad_model"), \
             patch("ultimate_asr.audio.recorder.SileroVAD", return_value=fake_vad):
            rec = Recorder(device_index=0)
